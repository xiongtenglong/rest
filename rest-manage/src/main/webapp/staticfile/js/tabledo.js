...
/* 实现表格序号列自动调整 created by  20081219 */
function sortnoTR(){
...
/* 设置此行是否更新 by  20120125 reason:按上面判断，有时无法触发，所以直接至标志；例如日期框
*/	    
    	...
    	for (j=0; j<newTR.cells.length; j++) {
    		...
    		if (inputArray.length > 0) {
	   			for (m=0; m<inputArray.length; m++) {
	   				// add by  20100315 reason:由于radio框名称相同,IE会解析为同一组；所以只能对其单独处理。通过动态替换radio的框的名字，使其随表各行新增行的行号发生变化。同时又由于radio的框的名称不同，提交后台处理时，又和其他文本框等处理不同，所以在radio后加了一个隐藏域，进行独特的处理。这样就和其他input框处理方式相同。
...
/* by  20100212 最后一行为空，则删除。reason:如果最后一行为空在删除,这样就不会因_CheckAll监测最后一行为空而提示未填写了 */
